...
In this part, we will deal with our missing data and use four kinds of graphs to see the distribution and percentage of our missing value. There are no missing values for country, development_regions, and sex, so we will focus on the rest of the variables.

```{r, echo=FALSE}
visdat::vis_miss(data) + 
  labs(title = "The percentage of missing data")
```
...
```{r, echo=FALSE}
gg_miss_var(data,
            facet = Development_Regions,show_pct = TRUE)+ labs(title = "The missing data with development regions")
```

...
library(rworldmap)
data_compare<-data_new%>%filter(country %in% countryRegions$ADMIN )%>%select(country,Sex,eduyears_2024_m)%>%pivot_wider(names_from = Sex,values_from=eduyears_2024_m)%>%mutate(difference=Male-Female)
...
op=par(mfrow=c(2,1))
mapCountryData(map, nameColumnToPlot="Female",borderCol='black',catMethod=c(1:16),colourPalette=brewer.pal(9,'YlOrBr'))
mapCountryData(map, nameColumnToPlot="Male",borderCol='black', catMethod=c(1:16),colourPalette=brewer.pal(9,'YlOrBr'))
```
...
```{r,echo=FALSE,warning=FALSE,fig.width=10}
mapCountryData(map, nameColumnToPlot="difference",borderCol='black',
